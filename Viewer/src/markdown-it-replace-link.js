// customised version of plugin that identifies images, so we can handle them differently
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (node.attribs) {
        if (node.name === 'img' && node.attribs.src) {
          node.attribs.src = replace(node.attribs.src, {...env, image:true}, token, node)
        }
            ...
            if (blockToken.type === 'inline' && blockToken.children) {
              blockToken.children.forEach(function (token) {
                ...
                if (type === 'link_open') {
                  ...
                  replaceAttr(token, 'src', replace, {...state.env, image:true})
                }
              })
            }
          })
